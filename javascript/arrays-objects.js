...
function myArr() {
  ...
  let arr = [4, `abc`, [`apple`, `berry`, `cherry`], true];
  return arr;
}
...
function flatAnimals() {
  ...
  const combined = [...forest, ...ocean, ...savannah, ...desert];
...
function betterCourses() {
  ...
  {
    for (let j = 0; j< courses[i].length; i++)
    {
      if (courses[i].inPerson === false)
      {
        courses[i].inPerson = true
      }
      else if (courses[i].homework === false)
      {
        courses[i].inPerson = true
      }
    }
...
{
  // Replace this with your code
  let {name, phoneNumber, email} = contactInfo
...
function getEllensInfo() {
  // Replace this with your code
  let obj = {...contactInfo, ...shippingInfo, name: 'Ellen', email: 'ellen@email.com'}
...
function lastTopic() {
  // Replace this with your code
  let lastItem = userInfo.topics.slice(-1).toString()
